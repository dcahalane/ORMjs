...
public class ORM extends HttpServlet {
    ...
    void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, SQLException {
        ...
        if ("CREATE".equals(action)) {
            ...
            if(tableName != null && key != null && val != null){
                try{
                    search(tableName, key, val, c, response); 
                }catch(Exception eSrc){
                    Logger.getLogger(ORM.class.getName()).log(Level.SEVERE, null, eSrc);
                }
            }
    ...
    void create(String tableName, Converter c, HttpServletResponse response) throws IOException, SQLException {
        if (tableName != null) {
    ...
    void search(String tableName, String key, String value, Converter c, HttpServletResponse response) throws IOException, SQLException, ClassNotFoundException {
        String json = c.search(tableName, key, value);
        Logger.getLogger(ORM.class.getName()).log(Level.INFO, "Found\t" + json);
        response.getWriter().println(json);
        response.getWriter().flush();
        response.getWriter().close();
    }
